...
//	dirctory = "/officekeepers13/tomcat/webapps/upload";
	...
	MultipartRequest multi = new MultipartRequest(request, dirctory, maxSize, encoding);
    	...
    	if (userID == null){
    		...
    		String lonelyDeathBBSIDStr = multi.getParameter("lonelyDeathBBSID");
    		
    		int lonelyDeathBBSID = Integer.parseInt(lonelyDeathBBSIDStr);
    		
    		//System.out.print("수정에서 BBSID값 : "+lonelyDeathBBSID);
...
//	        		} else{
		                ...
		                int result = lonelyDeathBBSDAO.update(lonelyDeathBBSID, lonelyDeathBBSTitle, userID, lonelyDeathBBSContent, fileName, fileRealName);
		                if (result == -1){ // 글쓰기 실패시
		                    PrintWriter script = response.getWriter();
		                    script.println("<script>");
